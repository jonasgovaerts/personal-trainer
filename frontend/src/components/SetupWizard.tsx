          ...
          {step === 1 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="text-center mb-8">
                <Dumbbell className="w-12 h-12 text-blue-500 mx-auto mb-4" />
                <h2 className="text-3xl font-bold text-white mb-2">{t('wizard.welcome')}</h2>
                <p className="text-slate-400">{t('wizard.subtitle')}</p>
              </div>

              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">{t('wizard.name')}</label>
                <input 
                  type="text" 
                  value={formData.name}
                  onChange={e => updateForm('name', e.target.value)}
                  placeholder="e.g. Jonas"
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl p-4 text-white focus:outline-none focus:border-blue-500 text-lg"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <button 
                  ...
                  className={cn("p-4 rounded-xl border transition-all font-semibold", formData.gender === 'male' ? "bg-blue-600/20 border-blue-500 text-white" : "bg-slate-950 border-slate-800 text-slate-400 hover:border-slate-700")}
                >
                  {t('profile.gender.male')}
                </button>
                <button 
                  onClick={() => updateForm('gender', 'female')}
                  className={cn("p-4 rounded-xl border transition-all font-semibold", formData.gender === 'female' ? "bg-blue-600/20 border-blue-500 text-white" : "bg-slate-950 border-slate-800 text-slate-400 hover:border-slate-700")}
                >
              ...
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">{t('wizard.birthDate')}</label>
                <input 
                  type="date" 
              ...
              <button 
                ...
                className="w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-bold py-4 rounded-xl transition-colors flex items-center justify-center gap-2 mt-4"
              >
          ...
          {step === 2 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
              <div className="text-center mb-8">
                ...
                <p className="text-slate-400">We use this to calculate your Base Metabolic Rate (BMR).</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">Current Weight (kg)</label>
                  <input 
                    type="number" 
                ...
                <div>
                  <label className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 block">Target Weight (kg)</label>
                  <input 
                    type="number" 
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button 
                  ...
                  className="flex-1 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-bold py-4 rounded-xl transition-colors flex items-center justify-center gap-2"
                >
          ...
          {step === 3 && (
            <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
              <div className="text-center mb-8">
                ...
                <p className="text-slate-400">How active are you on an average day?</p>
              </div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  >
                    <div>
                      <p className={cn("font-bold", formData.activityLevel === level.value ? "text-white" : "text-slate-300")}>{level.label}</p>
                      <p className="text-xs text-slate-500 mt-1">{level.desc}</p>
                    </div>
                    {formData.activityLevel === level.value && <Check className="w-5 h-5 text-blue-500" />}
                  </button>
                ))}
              </div>
              ...
              <div className="flex gap-4 pt-4">
                ...
                <button 
                  onClick={() => setStep(4)}
                  className="flex-1 bg-blue-600 hover:bg-blue-500 text-white font-bold py-4 rounded-xl transition-colors flex items-center justify-center gap-2"
                >
                  {t('wizard.next')} <ArrowRight className="w-5 h-5" />
                </button>
              </div>
